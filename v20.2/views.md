...
<span class="version-tag">New in v20.2:</span> CockroachDB supports [materialized views](https://en.wikipedia.org/wiki/Materialized_view). Materialized views and standard (i.e. "dematerialized") views share similar syntax for [creating](create-view.html), [showing](show-table.html), [renaming](alter-view.html), and [dropping](drop-view.html). However, unlike standard views, materialized views store their underlying query results.

When you [select](selection-queries.html) from a materialized view, the stored data is returned, which might be out-of-date. This contrasts with a standard view, which runs the underlying query every time it is used, returning the latest table results. In order to get the latest results from a materialized view, you must [refresh the view](refresh.html), and then select from it.
